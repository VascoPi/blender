...
class MxNodeTree(bpy.types.ShaderNodeTree):
    ...
    def output_node(self):
        return next((node for node in self.nodes
                     if node.bl_idname == 'usdhydra.MxNode_STD_surfacematerial'), None)

    @property
    def output_node_volume(self):
        return next((node for node in self.nodes
                     if node.bl_idname == 'usdhydra.MxNode_STD_volumematerial'), None)
    ...
    def create_basic_nodes(self, node_name='PBR_standard_surface'):
        ...
        def create_nodes():
            ...
            mat_node = self.nodes.new('usdhydra.MxNode_STD_surfacematerial')
            node = self.nodes.new(f'usdhydra.MxNode_{node_name}')
    ...
    def update_(self):
        ...
        for material in bpy.data.materials:
            if material.usdhydra.mx_node_tree and material.usdhydra.mx_node_tree.name == self.name:
                material.usdhydra.update()
